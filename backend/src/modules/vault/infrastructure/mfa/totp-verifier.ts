 ...
 * -----------------------
 *   This adapter is the *only* file in the vault module that imports
 *   `otpauth`. The application layer talks to the {@link TotpVerifier}
 *   port (`application/ports/totp-verifier.ts`) so swapping the
...
import type {
  ...
  TotpVerifyResult,
  TotpVerifier,
} from "../../application/ports/totp-verifier";
...
export class OtpAuthTotpVerifier implements TotpVerifier {
  async generateEnrollment(
    ...
    if (typeof actor !== "string" || actor.length === 0) {
      throw new Error(
        "[vault] OtpAuthTotpVerifier: actor must be a non-empty string.",
      );
    }
    if (typeof label !== "string" || label.length === 0) {
      throw new Error(
        "[vault] OtpAuthTotpVerifier: label must be a non-empty string.",
      );
    }
  ...
  async verifyCode(
    ...
    if (!Buffer.isBuffer(secret) || secret.length === 0) {
      throw new Error(
        "[vault] OtpAuthTotpVerifier: secret must be a non-empty Buffer.",
      );
    }
    if (typeof code !== "string") {
      throw new Error(
        "[vault] OtpAuthTotpVerifier: code must be a string.",
      );
    }
    if (!Number.isFinite(window) || window < 0) {
      throw new Error(
        "[vault] OtpAuthTotpVerifier: window must be ≥ 0.",
      );
    }
