 ...
 *   ever issued. The factory in `./index.ts` refuses to construct this
 *   adapter in `NODE_ENV=production` unless the operator has set
 *   `VAULT_ALLOW_UNSAFE_KEY_PROVIDER=true`; that override is treated
 *   as a deliberate, logged acceptance.
 *
 * Crypto:
...
import type {
  ...
  WrappedDek,
  WrappedSecret,
} from '../../application/ports/key-manager';
...
export class LocalDevKeyManager implements KeyManager {
  ...
  constructor(
    ...
    if (!Buffer.isBuffer(cfg.masterKey) || cfg.masterKey.length === 0) {
      throw new Error(
        '[vault] LocalDevKeyManager: masterKey must be a non-empty Buffer.',
      );
    }
    if (!cfg.keyVersion || typeof cfg.keyVersion !== 'string') {
      throw new Error(
        '[vault] LocalDevKeyManager: keyVersion is required.',
      );
    }
    ...
    if (cfg.acknowledgedUnsafe) {
      this.logger?.warn(
        ...
        'vault.key.local-dev.acknowledged-unsafe',
      );
    }
  }
  ...
  async generateDataKey(context: WrapContext): Promise<GeneratedDek> {
    if (!Buffer.isBuffer(context)) {
      throw new Error('[vault] LocalDevKeyManager: context must be a Buffer.');
    }
  ...
  async wrapDataKey(dek: PlainDek, context: WrapContext): Promise<WrappedDek> {
    if (!Buffer.isBuffer(dek) || dek.length !== DEK_LEN) {
      throw new Error(
        `[vault] LocalDevKeyManager: wrapDataKey requires a ${DEK_LEN}-byte DEK.`,
      );
    }
    if (!Buffer.isBuffer(context)) {
      throw new Error('[vault] LocalDevKeyManager: context must be a Buffer.');
  ...
  async unwrapDataKey(wrapped: WrappedDek, context: WrapContext): Promise<PlainDek> {
    if (!Buffer.isBuffer(wrapped?.bytes)) {
      throw new Error('[vault] LocalDevKeyManager: wrapped.bytes must be a Buffer.');
    }
    if (!Buffer.isBuffer(context)) {
      throw new Error('[vault] LocalDevKeyManager: context must be a Buffer.');
    }
    if (wrapped.bytes.length < IV_LEN + TAG_LEN + 1) {
      ...
      throw new Error('[vault] LocalDevKeyManager: wrapped blob is too short.');
    }
    ...
    try {
      ...
      throw new Error(
        `[vault] LocalDevKeyManager: unwrap integrity failure: ${(err as Error).message}`,
      );
    }
    if (plaintext.length !== DEK_LEN) {
      plaintext.fill(0);
      throw new Error(
        `[vault] LocalDevKeyManager: unwrapped DEK length mismatch (got ${plaintext.length}, expected ${DEK_LEN}).`,
      );
    }
    return plaintext;
  }
  ...
  async sealSecret(
    ...
    if (!Buffer.isBuffer(plaintext) || plaintext.length === 0) {
      throw new Error(
        '[vault] LocalDevKeyManager: sealSecret requires a non-empty Buffer.',
      );
    }
    if (!Buffer.isBuffer(context)) {
      throw new Error('[vault] LocalDevKeyManager: sealSecret context must be a Buffer.');
    }
  ...
  async openSecret(
    ...
    if (!Buffer.isBuffer(context)) {
      throw new Error('[vault] LocalDevKeyManager: openSecret context must be a Buffer.');
    }
    if (wrapped.bytes.length < IV_LEN + TAG_LEN + 1) {
      throw new Error('[vault] LocalDevKeyManager: wrapped blob is too short.');
    }
    ...
    try {
      ...
      throw new Error(
        `[vault] LocalDevKeyManager: openSecret integrity failure: ${(err as Error).message}`,
      );
    }
    return plaintext;
  }
