...
//       -> conceptIds                     (via CURRICULUM_MAPPING)
...
// the canonical curriculum framework. There is no level-number arithmetic, no topic-string
...
export interface LearningPathNode {
  /** Immutable curriculum concept tag, e.g. "S3.3". The only identity key. */
  conceptId: string;
  /** FLN level number this concept sits at. */
  level: number;
