...
//
//     npm run test:learning-path            (from backend/)
...
test('resolves the highest current curriculum level', () => {
  const highestLevel = Math.max(...allLevels);
  const highestConcept = CURRICULUM_MAPPING[highestLevel].conceptId;
  const path = buildLearningPath({ failedLevels: [highestLevel] });
  assert.ok(
    path.nodes.some((n) => n.conceptId === highestConcept && n.kind === 'gap'),
    `highest curriculum level L${highestLevel} was not resolved`,
  );
});
