...
export function registerDiagnosticBulkRoutes(app: express.Express) {
  ...
  app.post('/api/diagnostic/bulk', async (req, res) => {
    ...
    // Per-student generation lock. Filter out students who already have a
    // diagnostic paper for this cycle. The selective (single) path enforces
    // the same lock per call; this is the parallel pre-flight for the bulk
    // path. Locked students are reported back in `skipped` so the UI can
    // show 'Generated 5 of 7, skipped 2 (already placed)'.
    const existingLocks = await dbStore.getStudentCycleLocks();
    const allowedStudents: typeof paperStudents = [];
    const skippedStudents: { studentId: string; studentName: string; reason: string }[] = [];
    for (const s of paperStudents) {
      const attempt = recordStudentCycleLock(existingLocks, {
      ...
      if (attempt.ok) {
        ...
        // Cast to read existing — see the comment on the same pattern in
        // backend/src/routes/worksheets.ts:401. The narrowing should
        // work but doesn't in this tsc version for the surrounding try
        // block, so we read the field directly via a structural cast.
        const blocked = attempt as unknown as { existing: { generatedByEmail: string; createdAt: string } };
        skippedStudents.push({
          studentId: s.studentId,
          studentName: s.name,
          reason: `Already has a diagnostic paper (generated by ${blocked.existing.generatedByEmail} on ${blocked.existing.createdAt})`,
        });
      }
    }
    paperStudents = allowedStudents;
    paperCount = paperStudents.length;
    if (paperCount === 0) {
      return res.status(409).json({
        error: `Lock Active: All ${skippedStudents.length} selected students already have a diagnostic paper for this cycle. No papers generated.`,
    ...
    // Persist the locks for the students we're about to generate for.
    // The lock pre-flight above checked each one; this is the write
    // half of the operation. Done synchronously before the background
    // job starts so a crash in the worker doesn't leave the lock state
    // out of sync.
    for (const s of paperStudents) {
      const existingLocks = await dbStore.getStudentCycleLocks();
      const lockRecord = recordStudentCycleLock(existingLocks, {
        studentId: s.studentId,
        paperType: 'diagnostic',
        cycle: 'Baseline',
        generatedByEmail: user.email,
      ...
      if (lockRecord.ok) {
        await dbStore.addStudentCycleLock(lockRecord.lock);
      }
      // If the lock now exists (someone else just took it), skip the
      // student — the pre-flight already passed but the world may have
      // changed. We don't fail the whole batch.
    }
  ...
  app.post('/api/diagnostic/single', async (req, res) => {
    ...
    // Per-student generation lock (single-student diagnostic route).
    // Mirrors the same policy as the personalized-worksheet route:
    // one diagnostic paper per student, paperType='diagnostic',
    // cycle='Baseline'. The frontend also filters out placed students
    // from the dropdown (defense in depth), but this catches direct
    // API calls and stale UI state.
    const existingLocks = await dbStore.getStudentCycleLocks();
    ...
    if (!lockAttempt.ok) {
      ...
      return res.status(409).json({
        error: `Lock Active: This student already has a diagnostic paper (generated by ${blocked.existing.generatedByEmail} on ${blocked.existing.createdAt}). One per-student paper per cycle is allowed; re-generate is blocked.`,
        lockDetails: lockAttempt,
      });
    }
