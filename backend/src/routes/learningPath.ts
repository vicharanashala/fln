...
import { dbStore, EvaluationReport, Student, User, UserRole } from '../db';
...
export function registerLearningPathRoutes(app: express.Express) {
  ...
  // This is a teacher-mediated workflow; only the roles that have the Learning
  // Path UI may read or change it. canAccessStudent then applies their existing
  // school/assigned-school scope.
  function canManageLearningPath(user: User, student: Student): boolean {
    return (
      (user.role === UserRole.TEACHER || user.role === UserRole.VOLUNTEER) &&
      canAccessStudent(user, student)
    );
  }
  ...
  app.get('/api/students/:id/learning-path', async (req, res) => {
    ...
    if (!canManageLearningPath(user, student)) return res.status(403).json({ error: 'Forbidden.' });
  ...
  app.get('/api/students/:id/learning-path/nodes/:conceptId/practice', async (req, res) => {
    ...
    // Student-facing practice data must not expose answers in the browser.
    const questions = generateQuestionsForLevel(node.level, subLevel).map(({ answer, ...question }) => question);
    res.json({
