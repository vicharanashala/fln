    ...
    try {
      ...
      for (let sIdx = 0; sIdx < evalStudents.length; sIdx++) {
        ...
        diagQuestions.forEach((q: any) => {
          ...
          if (isMatch) topicStats[topic][diff].correct += 1;
        });
        ...
        if (Object.keys(conceptMastery).length === 0) {
          conceptMastery['Number Sense'] = percentage >= 100 ? 'Strong' : percentage >= 50 ? 'Satisfactory' : 'Needs Practice';
        }
        ...
        const report: EvaluationReport = {
          ...
          conceptMastery,
          topicStats,
          narrative: `ICR EasyOCR Answer Sheet Evaluation complete for ${student.name}.\n• Score: ${score}/${diagQuestions.length} (${percentage}%)\n• Assessed Level: Level ${recommendedLevel}.${subLevel}\n• Raw OCR Snippet: "${rawOcrText.slice(0, 60)}"`,
          recommendedLevel,
