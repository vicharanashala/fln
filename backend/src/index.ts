...
async function startServer() {
  ...
  app.get('/api/classes/:classId/growth', async (req, res) => {
    const user = getAuthUser(req);
    if (!user) return res.status(401).json({ error: 'Unauthorized' });
    ...
    const classStudents = students.filter(
      (s: any) => s.classGroup === classObj.className && s.section === classObj.section && s.schoolId === classObj.schoolId
    );

    const allReports = await dbStore.getEvaluationReports();

    const growthData = classStudents.map((student: any) => {
      const studentReports = allReports.filter((r: any) => r.studentId === student.id).sort((a: any, b: any) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime());
      ...
      if (studentReports.length === 0) {
        ...
        let lviBase = 40;
        studentReports.forEach((rep: any, idx: number) => {
