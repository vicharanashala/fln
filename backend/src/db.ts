...
export const connectDB = async () => {
  ...
  if (!uri) {
    console.warn("MONGODB_URI not set — running with file-based DB only");
    return;
  }
  try {
    ...
    console.warn("MongoDB connection failed — falling back to file-based DB:", err.message);
    mongoClient = null;
  }
};
...
export class DBStore {
  ...
  async getAnswerSubmissions() {
    ...
    return this.collection<AnswerSubmission>('answerSubmissions', 'answerSubmissions');
  }
  ...
  async updateStudent(studentId: string, updates: Partial<Student>) {
    ...
    if (this.data) {
      const idx = this.data.students.findIndex(x => x.id === studentId);
      if (idx !== -1) {
        this.data.students[idx] = { ...this.data.students[idx], ...updates } as Student;
        await this.save();
  ...
  async updateWorksheet(worksheetId: string, updates: Partial<Worksheet>) {
    ...
    if (this.data) {
      const idx = this.data.worksheets.findIndex(x => x.id === worksheetId);
      if (idx !== -1) {
        this.data.worksheets[idx] = { ...this.data.worksheets[idx], ...updates } as Worksheet;
        await this.save();
  ...
  async updateTicket(id: string, updates: Partial<Ticket>) {
    ...
    if (this.data) {
      const idx = this.data.tickets.findIndex(x => x.id === id);
      if (idx !== -1) {
        this.data.tickets[idx] = { ...this.data.tickets[idx], ...updates } as Ticket;
        await this.save();
  ...
  async updateSchool(schoolId: string, updates: Partial<School>) {
    ...
    if (this.data) {
      const idx = this.data.schools.findIndex(x => x.id === schoolId);
      if (idx !== -1) {
        this.data.schools[idx] = { ...this.data.schools[idx], ...updates } as School;
        await this.save();
  ...
  async getInterventions() {
    ...
    return this.collection<Intervention>('interventions', 'interventions');
  }

  async addIntervention(intervention: Intervention) {
    if (this.mongoDb) await this.mongoDb.collection('interventions').insertOne(intervention);
  ...
  async updateIntervention(id: string, updates: Partial<Intervention>) {
    ...
    if (this.data) {
      const idx = this.data.interventions.findIndex(x => x.id === id);
      if (idx !== -1) {
        this.data.interventions[idx] = { ...this.data.interventions[idx], ...updates } as Intervention;
        await this.save();
  ...
  async getBestPractices() {
    ...
    return this.collection<BestPractice>('bestPractices', 'bestPractices');
  }

  async addBestPractice(bp: BestPractice) {
    if (this.mongoDb) await this.mongoDb.collection('bestPractices').insertOne(bp);
  ...
  async updateBestPractice(id: string, updates: Partial<BestPractice>) {
    ...
    if (this.data) {
      const idx = this.data.bestPractices.findIndex(x => x.id === id);
      if (idx !== -1) {
        this.data.bestPractices[idx] = { ...this.data.bestPractices[idx], ...updates } as BestPractice;
        await this.save();
