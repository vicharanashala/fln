...
- **Teacher** — Classroom-level operator: manages their own class roster, triggers diagnostics, triggers AI-personalized worksheet generation, reviews reports, and conducts exams at schools with adequate student strength.
...
| Defaulter | A Teacher who has exhausted all 3 Delayed Attempts within an academic year and is blocked from generating/conducting exams for the remainder of that year (replaced by a Volunteer with Teacher Dashboard access; counter resets yearly) |
...
- Subject to the Delayed-Attempt/Defaulter escalation policy (§6.5): blocked from generating/conducting exams for the remainder of the academic year after 3 Delayed Attempts (counter resets yearly).
...
System triggers AI Diagnostic Test — NOT level-gated:
    ...
    not a sequence of per-level pass/fail gates
        │
        ▼
Student completes diagnostic (printed/scanned via ICR)
        │
        ▼
...
Volunteer OR Block Admin generates/prints the paper on-site
  (whichever generates first LOCKS the other out for that
   class/session — same Generation Lock mechanism, §13.2 R-11)
        │
        ▼
Volunteer travels to the school
...
If NOT submitted within the Submission Window:
  ...
  YES          NO → Teacher continues normally
   │
   ▼
  Teacher blocked from generating/conducting exams for the
  remainder of the academic year → replaced by a Volunteer,
  who is granted Teacher Dashboard access for that period
  (counter resets at the start of the next academic year)
        │
        ▼
...
| R-12 | **Delayed-Attempt/Defaulter escalation:** After 3 Delayed Attempts (submissions after the 1-hour window) within an academic year, a Teacher is blocked from generating/conducting exams for the remainder of that year. A Volunteer takes over with Teacher Dashboard access for that period. The Delayed Attempt counter resets at the start of each academic year. If all Teachers in a school default, School/Teacher dashboard access is reassigned to Block Admin/Volunteer; Admin or Superadmin may manually restore School access at any time |
...
- [ ] Delayed-attempt/defaulter escalation works: 3 Delayed Attempts within an academic year → Teacher blocked for the year → Volunteer takes over; counter resets yearly.
...
*End of SRS FLN Platform Document v0.1*
