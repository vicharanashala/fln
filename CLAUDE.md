...
│   ├── data/db.json          # JSON-file fallback store (MongoDB is the primary store when MONGODB_URI is set)
...
- Magic thresholds recur across many files: max level `109`, certification `currentLevel >= 5`, score bands `80/60`. If you change one, grep for the others (they are NOT centralized). AUDIT §3.3 lists locations.
- **MongoDB is real and primary when `MONGODB_URI` is set** (`backend/src/db.ts`, verified via `connectDB()`/`persistCollection()`); `backend/data/db.json` is the local-dev/no-Mongo fallback, not a sign Mongo is fake. Either way, writes go through a full delete-and-reinsert (Mongo) or full-file-rewrite (JSON) per mutation — not concurrency-safe under either backend.
...
- **`npm run lint` is `tsc --noEmit` — it proves types compile, nothing about behavior.** There is no test suite beyond the `backend/src/__checks__/` assert scripts. A green lint tells you nothing about whether a flow works; verify by running the app (`npm run dev:frontend`) and/or curl against the real backend (`npm run dev:backend`). Baseline note: two **pre-existing** type errors exist (`backend/src/index.ts:665`, `backend/src/paperGenerator.ts:233`) — don't add new ones.
...
- **Backend endpoints** → `backend/src/routes/<domain>.ts`, each exporting `register<Domain>Routes(app)` and registered in `backend/src/index.ts`. Reusable business logic → plain modules under `backend/src/services/`; data access through the existing `dbStore` in `backend/src/db.ts`. Do not introduce `modules/`, `controllers/`, `models/`, `repositories/`, or `interfaces/` layers, or a second database abstraction — this is the convention recorded in [docs/adr/001-backend-structure.md](docs/adr/001-backend-structure.md).
...
- **Never re-add `frontend/src/mock/`** — it is deleted. New data-fetching goes through the real API (`frontend/src/services/apiClient.ts`).
