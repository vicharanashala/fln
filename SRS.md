...
- **109 curriculum levels are currently defined** (`backend/src/config/curriculumMap.ts`), each mapped to a stage, age group, and concept ID. The count has moved twice — an older 59-level numbering was replaced by 93, which grew to 109 when Balvatika was finalized against NCF-FS 2022 — and is expected to move again as curriculum research continues; the level number is a display label over an immutable internal concept ID, not the canonical identifier.
- **Prerequisite structure is a DAG with AND/OR clauses, not a tree.** A **hard prerequisite is AND** — if a concept genuinely requires multiple specific prior skills, all are required. **Multiple valid routes to the same concept are OR** — a real alternative pathway (e.g. multiplication learnable via skip-counting OR times-tables), not a hedge for uncertainty. This lets two students who fail the same level be diagnosed differently depending on which prerequisite branch they're actually missing. Balvatika itself has almost no OR cases (~20+ of its ~30 nodes are independent with no prerequisite edges at all; only ~10-11 are AND-connected) — the OR case matters more from Class 1 onward.
- **Backward-mapping depth (how far below a failed level to place a student) follows a half-split rule**, matching Knowledge Space Theory (Falmagne et al.): don't drop a student straight to the bottom of the prerequisite chain on a wrong answer — move them roughly halfway down the chain below the failed level (e.g. 10 levels below → move down ~5; 20 levels below → move down ~10), proportional rather than a fixed count. **Not yet implemented** — depends on the topological-sort work over the prerequisite DAG.
- **Open, deliberately unsolved problem: how many questions per node are needed to call a concept "mastered," and how that interacts with worksheet length.** Standard mastery-assessment practice (and ASER's own norm) wants ~80% correct, i.e. 4-of-5 questions per concept — but Balvatika alone has ~30 nodes, so even 3 questions/node is a 90-question diagnostic, infeasible for a 5-6-year-old in one sitting. The usual fix (combining multiple nodes into one question) has limited reach for Balvatika specifically, since most of its nodes are independent rather than interconnected (see the AND/OR point above) — that compression works better once a stage has more prerequisite structure to lean on. FLN grading is pass/fail per concept, never a partial/numeric score, which constrains whatever threshold is eventually chosen. The same open question applies to teacher observation worksheets: how many times must a teacher observe a skill before ticking it off is also undecided. This is being deliberately researched before implementation, not guessed at under time pressure — do not hardcode a per-node question count or observation-count threshold until this is resolved.
