/**
 * `RequestDetokenization` command — application-layer use case (Session 7B).
 *
 ...
 *
 *   - The file knows about *domain* rules (token id must be a
 *     non-empty string, factor must be active, factor's actor must
 *     match the identity's actor, challenge TTL must be non-zero)
 *     and orchestrates the ports. It does NOT import any
 *     infrastructure adapter (`pg`, `node:crypto`, etc.).
 ...
 *     rolled-back unit-of-work cannot emit a phantom event to
 *     subscribers.
 *
...
import type { DomainEvent, EventPublisher } from '../ports/event-publisher.js';
 ...
 * challenge becomes invalid. After that point both `VerifyMfa`
 * and `DetokenizeAadhaar` will refuse it with
 * `CHALLENGE_EXPIRED`.
 *
...
export function makeRequestDetokenization(deps: RequestDetokenizationDeps) {
    ...
    return async function requestDetokenization(
        ...
        if (ttlSeconds <= 0) {
            // Defensive — a misconfigured factory should fail loudly
            // here so the caller doesn't get a challenge that
            // expires instantly (or, worse, a negative TTL that
            // means "expired in the past" and is permanently dead).
            throw new RequestDetokenizationCommandError(
                'INVALID_CONFIG',
        ...
        // -----------------------------------------------------------------
        // 3. Load the parent identity row. We need it both for the
        //    actor-ownership check on the factor below AND to bind
        //    the challenge to the identity (challenge.identityId is
        //    the audit/replay anchor used by VerifyMfa and
        //    DetokenizeAadhaar).
        // -----------------------------------------------------------------
        ...
        // (No actor-ownership comparison here: the `IdentityRecord`
        // schema does not carry an actor field. Ownership is
        // implicitly enforced later: the challenge row pins
        // `requiredFactorId`, and the matching `DetokenizeAadhaar`
        // call will only succeed if the caller proves possession
        // of that exact factor in the `VerifyMfa` step. A caller
        // who picks someone else's factor cannot complete the
        // step-up.)
        ...
        // -----------------------------------------------------------------
        // 6. Append the audit row. Action=`STEP_UP_REQUEST`,
        //    outcome=`allow` (a refused creation has already
        //    thrown above).
        // -----------------------------------------------------------------
