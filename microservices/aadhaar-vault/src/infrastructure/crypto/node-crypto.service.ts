/**
 * NodeCryptoService — AES-256-GCM adapter (Session 4).
 *
 ...
 * use the `dek-zero` helper (Session 3) at the right boundary.
 */
...
import type {
    CryptoService,
    EncryptedEnvelope,
} from '../../application/ports/crypto.service.js';
...
function assertKeyLength(key: Buffer): void {
    if (key.length !== KEY_LEN) {
        throw new Error(
            `[aadhaar-vault] crypto: AES-256 requires a ${KEY_LEN}-byte key; got ${key.length}.`,
        );
    }
}
...
function assertEnvelopeShape(env: EncryptedEnvelope): void {
    if (env.iv.length !== IV_LEN) {
        throw new Error(
            `[aadhaar-vault] crypto: iv must be ${IV_LEN} bytes; got ${env.iv.length}.`,
        );
    }
    if (env.authTag.length !== TAG_LEN) {
        throw new Error(
            `[aadhaar-vault] crypto: authTag must be ${TAG_LEN} bytes; got ${env.authTag.length}.`,
        );
    }
}