...
(async () => {
  ...
  await c.connect();
  const db = c.db();
  const s = await db.collection('schools').deleteMany({ id: { $in: ['gps-i3-test-001'] } });
  const u = await db.collection('users').deleteMany({ email: 'p.i3@fln.org' });
  const l = await db.collection('logs').deleteMany({ schoolId: 'gps-i3-test-001' });
