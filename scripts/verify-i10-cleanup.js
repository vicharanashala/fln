...
(async () => {
  ...
  await c.connect();
  const db = c.db();
  const s = await db.collection('schools').deleteMany({ id: { $in: ['gps-i10-A', 'gps-i10-B'] } });
  const u = await db.collection('users').deleteMany({ email: { $in: ['t.i10.a@fln.org', 't.i10.b@fln.org'] } });
  const l = await db.collection('logs').deleteMany({ schoolId: { $in: ['gps-i10-A', 'gps-i10-B'] } });
