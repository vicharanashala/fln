...
def detect_page_markers(image: Image.Image) -> dict[str, Any]:
    ...
    try:  # pragma: no cover - cv2 is optional in the local test runtime
        ...
        # RETR_LIST is required because printable papers may have an outer page
        # border that encloses all four fiducials. RETR_EXTERNAL would expose
        # only that border and hide the marker contours inside it.
