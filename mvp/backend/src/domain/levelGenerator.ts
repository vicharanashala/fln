/**
 * Deterministic FLN curriculum question generator (59 levels × 3 sub-levels).
 *
 * Ported verbatim from the original prototype's generator — this is the good,
 * curriculum-faithful core. It is now the SINGLE source of truth (the duplicate
 * that lived in the frontend has been removed). Consumers map `RawQuestion` to
 * the persisted Worksheet question shape via `domain/levels.ts`.
 */
export interface RawQuestion {
  question_id: string;
  question: string;
  answer: string;
  answer_type: 'number' | 'text' | 'choice';
  choices?: string[];
  topic: string;
  subtopic: string;
