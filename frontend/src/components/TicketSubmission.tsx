            ...
            <div className="space-y-3">
              {displayTickets.map((t) => (
                <div key={t.id} className="bg-white dark:bg-slate-900 p-5 border border-zinc-200 dark:border-zinc-700 rounded-xl shadow-sm space-y-3">
                  ...
                  <p className="text-zinc-600 dark:text-zinc-300 text-xs leading-relaxed bg-zinc-50 dark:bg-zinc-800/50 p-3 rounded-lg border border-zinc-100 dark:border-zinc-700">{t.description}</p>
                      ...
                      {expandedAppeals.includes(t.id) && t.metadata?.logbookContext && (
                        <div className="mt-3 space-y-2 border-l-2 border-indigo-200 dark:border-indigo-800 pl-3">
                          {t.metadata.logbookContext.length === 0 ? (
                            <div className="text-xs text-zinc-500 dark:text-zinc-400 italic">No recent logbook activity found.</div>
                          ) : (
                            t.metadata.logbookContext.map(log => (
                              <div key={log.id} className="text-[10px] bg-slate-50 dark:bg-slate-800/50 p-2 rounded flex justify-between border border-transparent dark:border-slate-700/50">
                                <span className="font-mono text-slate-500 dark:text-slate-400">{new Date(log.timestamp).toLocaleString()}</span>
