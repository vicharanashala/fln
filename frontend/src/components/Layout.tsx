  ...
  return (
    <div className={`flex min-h-screen flex-col font-sans bg-slate-50 text-slate-900 antialiased ${darkMode ? 'dark bg-slate-950 text-slate-100' : ''}`}>
      ...
      <div className="flex flex-1 items-start">
        ...
        {/* Sidebar Left panel */}
        <aside className={`sticky top-20 h-[calc(100vh-5rem)] hidden md:flex flex-col border-r border-slate-200 bg-white transition-all duration-200 ${collapsed ? 'w-17' : 'w-64'}`}>
                ...
                <div key={item.name} className="space-y-1">
                  <div className="flex items-center justify-between group">
                    <button
                      ...
                      className={`relative flex ${collapsed ? '' : 'flex-1'} items-center gap-3 rounded-lg px-3 py-2.5 text-xs font-bold transition duration-150 border ${isSelected
                    ...
                    >
                      ...
                      {item.badge && (
                        <span className={
                          collapsed
                            ? "absolute top-1 right-1 flex h-3.5 w-3.5 items-center justify-center rounded-full bg-rose-500 text-[9px] font-bold text-white shadow-sm"
                            : "ml-auto bg-rose-500 text-white font-mono text-[9px] px-1.5 py-0.5 rounded-full"
                        }>
                          {item.badge}
                        </span>
                      )}
        ...
        {mobileOpen && (
          <div className="fixed inset-0 z-40 flex md:hidden bg-slate-900/50 backdrop-blur-sm">
            <div className="w-72 bg-white flex flex-col h-full shadow-2xl p-4 animate-slideIn">
              ...
              <nav className="flex-1 mt-autospace-y-1.5 overflow-y-auto">
                        ...
                        <div className="pl-6 space-y-1.5 py-1 border-l border-slate-200 ml-5">
                          {item.subItems?.map(sub => (
                            <button
                              ...
                              className={`flex w-full items-center gap-2 px-3 py-1.5 text-xs font-medium rounded-md transition ${activeView === sub.view ? 'text-indigo-600 bg-indigo-50' : 'text-slate-500'
                                }`}
                            >
                              <span>{sub.name}</span>
                            </button>
