...
import { getBlockSuggestions, fetchDistrictCode } from '../utils/geoLookup';

interface BlockAutocompleteProps {
  value: string;
  onChange: (val: string, selectedBlock?: { code: string; name: string; districtCode: string }) => void;
...
export const BlockAutocomplete: React.FC<BlockAutocompleteProps> = ({
  ...
  className = '',
  required = false,
  title = 'Type or select Block Code',
  id,
}) => {
  const [isOpen, setIsOpen] = useState(false);
  const wrapperRef = useRef<HTMLDivElement>(null);

  const suggestions = getBlockSuggestions(value, parentDistrict, parentState);
  const distDetails = fetchDistrictCode(parentDistrict, parentState);
  const distLabel = distDetails.name || parentDistrict || 'Selected District';
  ...
  const handleSelect = (item: { code: string; name: string; districtCode: string }) => {
      ...
      {isOpen && (
        <div className="absolute left-0 right-0 z-50 mt-1 max-h-56 overflow-y-auto rounded-lg border border-slate-200 dark:border-zinc-700 bg-white dark:bg-slate-900 shadow-xl transition-all font-sans">
          <div className="px-3 py-1.5 bg-slate-50 dark:bg-zinc-800 border-b border-slate-100 dark:border-zinc-700 text-[10px] font-semibold text-slate-500 dark:text-zinc-400 flex items-center justify-between">
            <span>BLOCK SUGGESTIONS ({distLabel})</span>
          ...
          {suggestions.length === 0 ? (
            <div className="p-3 text-xs text-slate-400 dark:text-zinc-500 text-center italic">
              {!parentDistrict
                ? 'Please select a district first to view block suggestions'
                : `No blocks matching "${value}" found under ${distLabel}`}
            </div>
          ) : (
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] text-slate-400 dark:text-zinc-500">({item.districtName})</span>
                    </div>
