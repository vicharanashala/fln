...
return <SuperadminDashboard user={currentUser} token={token!} />;
      ...
      case 'teacher':
        return <TeacherDashboard user={currentUser} token={token!} onSelectView={setActivePanel} />;
      case 'volunteer':
          ...
          <div className="flex min-h-screen flex-col font-sans bg-slate-50 text-slate-900 dark:bg-slate-950 dark:text-slate-100 antialiased">
            ...
            {currentView === 'dashboard' && currentUser && token && (
              ...
              >
                ...
                {activePanel === 'interventions' && <InterventionTracking token={token} userRole={currentUser.role} onSelectView={setActivePanel} />}
                {activePanel === 'best-practices' && <BestPractices token={token} />}
