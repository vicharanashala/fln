                ...
                {activePanel === 'settings' && (
                  <div className="space-y-6 rounded-xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-900">
                    ...
                    <div className="grid grid-cols-1 gap-6 text-sm font-sans md:grid-cols-2">
                      <div className="space-y-4">
                        <h3 className="font-mono text-xs font-bold uppercase text-slate-800 dark:text-slate-200">User Profile Details</h3>
                        <div className="space-y-2 rounded-lg border border-slate-150 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-800/50">
                          <div><span className="text-xs font-semibold text-slate-450 dark:text-slate-400">Full Name:</span> <strong className="text-slate-800 dark:text-white">{currentUser.name}</strong></div>
                          <div><span className="text-xs font-semibold text-slate-450 dark:text-slate-400">Email ID:</span> <strong className="font-mono text-slate-850 dark:text-white">{currentUser.email}</strong></div>
                          <div><span className="text-xs font-semibold text-slate-450 dark:text-slate-400">Assigned Scope:</span> <strong className="font-mono text-slate-800 dark:text-white">{currentUser.schoolId || currentUser.districtCode || currentUser.stateCode || 'National Oversight'}</strong></div>
                        </div>
                      </div>
                      <div className="space-y-4">
                        <h3 className="font-mono text-xs font-bold uppercase text-slate-800 dark:text-slate-200">Accessibility Configuration</h3>
                        <div className="space-y-3 rounded-lg border border-slate-150 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-800/50">
                          <label className="flex items-center gap-2 font-medium dark:text-slate-200">
                            <input type="checkbox" defaultChecked className="rounded border-slate-300 text-indigo-650 dark:border-slate-600 dark:bg-slate-700" />
                            <span>Enable High-Contrast Border Outlines</span>
                          </label>
                          <label className="flex items-center gap-2 font-medium dark:text-slate-200">
                            <input type="checkbox" className="rounded border-slate-300 text-indigo-650 dark:border-slate-600 dark:bg-slate-700" />
