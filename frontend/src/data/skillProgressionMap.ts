// 109 FLN Levels → 24 Core Skills (SK01-SK24) → ~150 Granular Subskills
// Source: docs/skill-graph/FLN_93_Level_Skill_Graph_Specification.md
//         (the canonical spec the project adopted Sep 2026 — filename predates
//         the 2026-09-19 Balvatika sync, see below; node count has since grown)
//
// Synced 2026-09-19 (issue #522) to backend/src/config/curriculumMap.ts's
// 109-level registry: 15 new Balvatika nodes (11 split off later-stage nodes,
// 4 new), 4 nodes moved in from Class 1/2 (S-code kept, stage label only),
// S3.4 re-scoped, S3.6 split (new S1.8), S3.9 moved to Class 1 — all per
// PR #517/#519 and Pavani's 2026-09-19 NCF-FS-final ruling. Generated by a
// validated script (parse -> remap -> validate -> write), not hand-edited —
// every prerequisite levelId and skill reference was checked to resolve
// in-memory before this file was written. The 15 new/split nodes' primary/
// supporting skill assignments and prerequisite edges were authored fresh for
// this sync (ported from the already NCF-FS-sourced backend edges in
// competencyPrerequisites.ts, not re-derived) — worth a curriculum-lead
// spot-check on the SK assignments specifically, since those are new
// judgment calls this pass made, unlike the prerequisite edges which just
// carry over already-decided backend data.
//
...
// Levels L47-L109 (Class 1 → Class 4) are now also mapped (L28-L46 predates
// the 2026-09-19 Balvatika sync -- see that note above for what moved where).
...
// ─────────────────────────────────────────────────────────────────────────────
// 109 Levels (Spec §4, extended 2026-09-19): levelId, capability, primary skills, supporting skills.
...
// Cumulative level counts per S-notation stage (S1..S7): 8, 10, 28, 14, 17,
// 14, 18 — sums to 109. Updated 2026-09-19 (issue #522) to match
...
function stageIndexFor(n: number): number {
  ...
  throw new Error(`Level ${n} is outside the 109-level range (1-109).`);
}

// Direct position -> sCode lookup, generated 2026-09-19 (issue #522) from
// backend/src/config/curriculumMap.ts's own ordering. Replaces the previous
// pure position-arithmetic sCodeFor() (`S${stage}.${n - prevBoundary}`),
// which assumed every stage's S-codes are dense/gapless -- true before this
// sync, but S3.9 moving out to Class 1 (see that entry's comment) leaves a
// real gap in Stage 3's numbering (...S3.8, S3.10, S3.11...) that arithmetic
// alone cannot reproduce. Caught by this file's own sanity() companion check
// when getLevelBySCode('S3.9') returned the wrong node (position 27's real
// conceptId is S3.10, but position-arithmetic computed "S3.9" for it) --
// fixed by sourcing sCode directly from data instead of deriving it.
const POSITION_TO_SCODE: readonly string[] = [
  'S1.1', 'S1.2', 'S1.3', 'S1.4', 'S1.5', 'S1.6', 'S1.7', 'S1.8',
  'S2.1', 'S2.2', 'S2.3', 'S2.4', 'S2.5', 'S2.6', 'S2.7', 'S2.8',
  'S2.9', 'S2.10', 'S3.1', 'S3.2', 'S3.3', 'S3.4', 'S3.5', 'S3.6',
  'S3.7', 'S3.8', 'S3.10', 'S3.11', 'S3.12', 'S3.13', 'S3.14', 'S3.15',
  'S3.16', 'S3.17', 'S3.18', 'S3.19', 'S3.20', 'S3.21', 'S3.22', 'S3.23',
  'S3.24', 'S3.25', 'S4.12', 'S4.13', 'S5.9', 'S5.13', 'S4.1', 'S4.2',
  'S4.3', 'S4.4', 'S4.5', 'S4.6', 'S4.7', 'S4.8', 'S3.9', 'S4.9',
  'S4.10', 'S4.11', 'S4.14', 'S4.15', 'S5.1', 'S5.2', 'S5.3', 'S5.4',
  'S5.5', 'S5.6', 'S5.7', 'S5.8', 'S5.10', 'S5.11', 'S5.12', 'S5.14',
  'S5.15', 'S5.16', 'S5.17', 'S5.18', 'S5.19', 'S6.1', 'S6.2', 'S6.3',
  'S6.4', 'S6.5', 'S6.6', 'S6.7', 'S6.8', 'S6.9', 'S6.10', 'S6.11',
  'S6.12', 'S6.13', 'S6.14', 'S7.1', 'S7.2', 'S7.3', 'S7.4', 'S7.5',
  'S7.6', 'S7.7', 'S7.8', 'S7.9', 'S7.10', 'S7.11', 'S7.12', 'S7.13',
  'S7.14', 'S7.15', 'S7.16', 'S7.17', 'S7.18',
];

/** The level at position n's S-code, sourced from POSITION_TO_SCODE, e.g. sCodeFor(47) === "S4.1". */
function sCodeFor(n: number): string {
  const sCode = POSITION_TO_SCODE[n - 1];
  if (!sCode) throw new Error(`No sCode recorded for position ${n} -- POSITION_TO_SCODE is out of sync with curriculumMap.ts`);
  return sCode;
}
...
export const LEVEL_SKILL_MAP: LevelSkillMapping[] = [
  ...
  makeLevel(6, "Shape Matching (Perceptual, Any Size/Orientation)", ['SK19'], ['SK04'],
  ...
  makeLevel(8, "Arranges Familiar Events/Objects in Sequence", ['SK03'], ['SK04'],
    {  }),

  // ── Pre-school 2 (L9-L18) ──────────────────────────────────────────────────────────
  makeLevel(9, "Quantity Comparison", ['SK09'], ['SK01','SK06'],
    { prerequisites: [
      { levelId: 'L1', relationshipType: 'required_for_procedure',
  ...
  makeLevel(10, "Seriation (3 Objects)", ['SK03'], ['SK09'],
    { prerequisites: [
      { levelId: 'L9', relationshipType: 'required_for_procedure',
        rationale: "Seriation requires repeated pairwise comparison" }],
    evidence: ['seriate_3'] }),
  makeLevel(11, "Classification (Increasing Complexity)", ['SK02'], ['SK04'],
    { prerequisites: ['L2'], relationshipType: 'supports',
    evidence: ['multi_property_classify'] }),
  makeLevel(12, "Counting to 5 (Cardinality)", ['SK05','SK06'], ['SK01'],
    { prerequisites: ['L5'], relationshipType: 'required_for_procedure',
    evidence: ['cardinality_to_5'] }),
  makeLevel(13, "Counting 6-10", ['SK05'], ['SK06'],
    { prerequisites: ['L12'], relationshipType: 'often_precedes',
    evidence: ['count_to_10'] }),
  makeLevel(14, "Shape Identification", ['SK19'], ['SK04'],
    { prerequisites: ['L6'], relationshipType: 'supports',
    evidence: ['shape_id_correct'] }),
  makeLevel(15, "2-Item Patterns", ['SK18'], ['SK04'],
    { prerequisites: ['L11'], relationshipType: 'often_precedes',
    evidence: ['ab_pattern_extend'] }),
  makeLevel(16, "Comparative Vocabulary", ['SK20'], ['SK09'],
    { prerequisites: ['L9'], relationshipType: 'supports',
    evidence: ['compare_vocab_correct'] }),
  makeLevel(17, "Conceptual Subitizing (to 6)", ['SK07'], ['SK06','SK12'],
    { prerequisites: [
      { levelId: 'L7', relationshipType: 'required_for_procedure',
        rationale: "Clements & Sarama: perceptual subitizing precedes conceptual" },
      { levelId: 'L12', relationshipType: 'supports' }],
    evidence: ['conceptual_subitize'] }),
  makeLevel(18, "Basic Shape Composition", ['SK19'], ['SK12'],
    { prerequisites: ['L14'], relationshipType: 'supports',
    evidence: ['compose_shape'] }),

  // ── Pre-school 3 / Balvatika (Age 5-6) -- finalised against NCF-FS, PR #517/#519 (L19-L46) ──
  makeLevel(19, "Numeral Recognition (1-10)", ['SK08'], ['SK05'],
    { prerequisites: ['L4'], relationshipType: 'often_precedes',
    evidence: ['numeral_id_1_10'] }),
  makeLevel(20, "Numeral-Quantity Correspondence", ['SK08','SK06'], ['SK05'],
    { prerequisites: [
      ...
      { levelId: 'L19', relationshipType: 'required_for_procedure' }],
    evidence: ['numeral_quantity_match'] }),
  makeLevel(21, "Numeral Comparison (Object-Mediated)", ['SK09'], ['SK08'],
    { prerequisites: ['L20'], relationshipType: 'often_precedes',
    evidence: ['numeral_compare_correct'] }),
  makeLevel(22, "Seriation (Up to 5 Objects, by Size/Length/Weight)", ['SK03'], ['SK09'],
    { prerequisites: [
      { levelId: 'L10', relationshipType: 'required_for_procedure',
        rationale: "Ordering 5 objects extends the same pairwise-comparison skill as ordering 3" }],
    evidence: ['seriate_5'] }),
  makeLevel(23, "Flexible Classification", ['SK02'], ['SK04','SK24'],
    { prerequisites: [
  ...
  makeLevel(24, "Numeral Sequencing", ['SK10'], ['SK05','SK08'],
    { prerequisites: [
      { levelId: 'L19', relationshipType: 'required_for_procedure',
        rationale: "Cannot sequence numeral symbols that aren't yet recognized" }],
    evidence: ['sequence_complete'] }),
  makeLevel(25, "Comparative Vocabulary (Formalizing)", ['SK20'], ['SK09'],
    { prerequisites: ['L16'], relationshipType: 'supports',
    evidence: ['formal_compare_vocab'] }),
  makeLevel(26, "Patterns (2-Item Indep & 3-Item Intro)", ['SK18'], ['SK04'],
    { prerequisites: ['L15'], relationshipType: 'often_precedes',
  ...
  makeLevel(28, "Number Names to 20 (Rote)", ['SK05'], ['SK10'],
    { prerequisites: [
      { levelId: 'L4', relationshipType: 'required_for_procedure',
        rationale: "NCF-FS C-8.3: number names to 20 extends rote counting to 10" }] }),
  makeLevel(29, "Counts in Any Order (Order Irrelevance)", ['SK06'], ['SK05'],
    { prerequisites: [
      { levelId: 'L12', relationshipType: 'required_for_procedure',
        rationale: "Gelman & Gallistel (1978) order-irrelevance; NCF-FS C-8.3" }] }),
  makeLevel(30, "Writes Numerals to 9", ['SK08'], ['SK05'],
    { prerequisites: [
      { levelId: 'L19', relationshipType: 'required_for_procedure',
        rationale: "NCF-FS C-8.5: writing numerals extends recognition" }] }),
  makeLevel(31, "Combines Groups to 9 (Object-Based, No Symbol)", ['SK13'], ['SK06'],
    { prerequisites: [
      { levelId: 'L13', relationshipType: 'required_for_procedure',
        rationale: "NCF-FS C-8.6: object-based combining, no + symbol yet" }] }),
  makeLevel(32, "Takes Away to 9 (Object-Based, No Symbol)", ['SK14'], ['SK06'],
    { prerequisites: [
      { levelId: 'L13', relationshipType: 'required_for_procedure',
        rationale: "NCF-FS C-8.6: object-based taking-away, no - symbol yet" }] }),
  makeLevel(33, "Makes Groups & Counts Objects/Groups", ['SK15'], ['SK05'],
    { prerequisites: [
      { levelId: 'L13', relationshipType: 'required_for_procedure',
        rationale: "NCF-FS C-8.7: making/counting groups precedes multiplication facts" }] }),
  makeLevel(34, "Shares Objects Equally (Up to 20, Among 4-5)", ['SK16'], ['SK01'],
    { prerequisites: [
      { levelId: 'L1', relationshipType: 'required_for_procedure',
        rationale: "Frydman & Bryant (1988): sharing by one-to-one dealing" }] }),
  makeLevel(35, "Creates a New Pattern", ['SK18'], ['SK04'],
    { prerequisites: [
      { levelId: 'L26', relationshipType: 'required_for_procedure',
        rationale: "NCF-FS C-8.2: creates a new pattern by feature" }] }),
  makeLevel(36, "Describes a Repeating Pattern's Rule", ['SK18'], ['SK24'],
    { prerequisites: [
      { levelId: 'L26', relationshipType: 'required_for_procedure',
        rationale: "NCF-FS C-8.2: describes a repeating pattern rule" }] }),
  makeLevel(37, "Describes 3D Solids in Own Words", ['SK19'], ['SK04'],
    { prerequisites: [
      { levelId: 'L14', relationshipType: 'required_for_procedure',
        rationale: "NCF-FS C-8.8: informal description of solids" }] }),
  makeLevel(38, "Traces Faces of 3D Objects", ['SK19'], ['SK04'],
    { prerequisites: [
      { levelId: 'L14', relationshipType: 'required_for_procedure',
        rationale: "NCF-FS C-8.8: tracing 3D faces on a plane" }] }),
  makeLevel(39, "Draws 2D Shapes Freehand", ['SK19'], ['SK04'],
    { prerequisites: [
      { levelId: 'L14', relationshipType: 'required_for_procedure',
        rationale: "NCF-FS C-8.8: freehand 2D drawing" }] }),
  makeLevel(40, "Compares Capacity of Two Vessels", ['SK20'], ['SK09'],
    {  }),
  makeLevel(41, "Names Days of the Week & Months", ['SK21'], ['SK05'],
    {  }),
  makeLevel(42, "Solves Simple Number Riddles/Puzzles", ['SK24'], ['SK08'],
    {  }),
  makeLevel(43, "Concept of Zero", ['SK08'], ['SK05','SK11'],
    { prerequisites: ['L19'], relationshipType: 'often_precedes' }),
  makeLevel(44, "Ordinal Positions (1st-10th)", ['SK10'], ['SK05'],
    { prerequisites: [
      { levelId: 'L24', relationshipType: 'required_for_procedure',
        rationale: "Ordinal position builds on the general ability to order things" }] }),
  makeLevel(45, "Currency Recognition", ['SK22'], ['SK08'],
    { prerequisites: ['L61'], relationshipType: 'often_precedes' }),
  makeLevel(46, "Spatial Vocabulary", ['SK19'], ['SK24'],
    { prerequisites: ['L54'], relationshipType: 'supports' }),

  // ── Class 1 (L47-L60) ───────────────────────────────────────────────────────────────
  makeLevel(47, "Abstract Numeral Comparison", ['SK09'], ['SK08','SK10'],
    { prerequisites: [
      { levelId: 'L21', relationshipType: 'required_for_procedure',
        rationale: "Object-mediated comparison is the direct predecessor of abstract comparison" }] }),
  makeLevel(48, "Close Numeral Comparison", ['SK09'], ['SK11'],
    { prerequisites: ['L47'], relationshipType: 'often_precedes' }),
  makeLevel(49, "Counting Objects to 20", ['SK05','SK06'], ['SK01'],
    { prerequisites: [
      { levelId: 'L12', relationshipType: 'required_for_procedure',
  ...
  makeLevel(50, "Reading & Writing Numerals to 99", ['SK08'], ['SK05','SK11'],
    { prerequisites: [
      { levelId: 'L19', relationshipType: 'required_for_procedure',
  ...
  makeLevel(51, "Tens and Ones", ['SK11'], ['SK12'],
    { prerequisites: ['L50'], relationshipType: 'required_for_procedure' }),
  makeLevel(52, "Single-Digit Addition", ['SK13'], ['SK05','SK06','SK12'],
    { prerequisites: [
      { levelId: 'L51', relationshipType: 'often_precedes',
  ...
  makeLevel(53, "Single-Digit Subtraction", ['SK14'], ['SK05','SK06'],
    { prerequisites: ['L52'], relationshipType: 'required_for_procedure' }),
  makeLevel(54, "3D Shape Properties", ['SK19'], ['SK04'],
    { prerequisites: ['L55'], relationshipType: 'supports' }),
  makeLevel(55, "2D Shape Identification & Properties (Named Shapes)", ['SK19'], ['SK04'],
    { prerequisites: ['L14'], relationshipType: 'supports',
    evidence: ['shape_property_id'] }),
  makeLevel(56, "Non-Standard Length Estimation", ['SK20'], ['SK24'],
    { prerequisites: ['L25'], relationshipType: 'supports' }),
  makeLevel(57, "Non-Standard Capacity Estimation", ['SK20'], ['SK24'],
    { prerequisites: ['L56'], relationshipType: 'supports' }),
  makeLevel(58, "3-Item Pattern Completion", ['SK18'], ['SK04'],
    { prerequisites: ['L26'], relationshipType: 'often_precedes' }),
  makeLevel(59, "Informal Number Line (0-20)", ['SK10'], ['SK09'],
    { prerequisites: ['L49'], relationshipType: 'often_precedes' }),
  makeLevel(60, "Advanced Shape Composition", ['SK19'], ['SK12'],
    { prerequisites: [
      { levelId: 'L27', relationshipType: 'required_for_procedure',
        rationale: "Same trajectory: Shape Composer follows Picture Maker" }] }),

  // ── Class 2 (L61-L77) ───────────────────────────────────────────────────────────────
  makeLevel(61, "Reading & Writing 3-Digit Numbers", ['SK08'], ['SK11'],
    { prerequisites: [
  ...
  makeLevel(62, "Tens as Bundles/Groups", ['SK11'], ['SK12'],
    { prerequisites: ['L51'], relationshipType: 'often_precedes' }),
  makeLevel(63, "Flexible 2-Digit Decomposition", ['SK12'], ['SK11'],
    { prerequisites: [
  ...
  makeLevel(64, "2-Digit Addition with Regrouping", ['SK13','SK11'], ['SK12'],
    { prerequisites: [
      { levelId: 'L52', relationshipType: 'required_for_procedure' },
      { levelId: 'L63', relationshipType: 'required_for_procedure',
        rationale: "Can't regroup without decomposition" }] }),
  makeLevel(65, "2-Digit Subtraction with Regrouping", ['SK14','SK11'], ['SK12'],
    { prerequisites: [
      { levelId: 'L53', relationshipType: 'required_for_procedure' },
      { levelId: 'L63', relationshipType: 'required_for_procedure',
        rationale: "Can't regroup without decomposition (same as the addition case)" }] }),
  makeLevel(66, "Multiplication as Repeated Addition", ['SK15'], ['SK13','SK18'],
    { prerequisites: ['L52','L77'], relationshipType: 'often_precedes' }),
  makeLevel(67, "Division as Equal Sharing", ['SK16'], ['SK01','SK06'],
    { prerequisites: ['L52'], relationshipType: 'often_precedes' }),
  makeLevel(68, "Multiplication Tables (2,3,4,5,10)", ['SK15'], ['SK18'],
    { prerequisites: ['L66'], relationshipType: 'required_for_procedure' }),
  makeLevel(69, "Informal Fractions (Folding)", ['SK17'], ['SK19'],
    { prerequisites: ['L60'], relationshipType: 'supports' }),
  makeLevel(70, "Uniform Non-Standard Measurement", ['SK20'], ['SK24'],
    { prerequisites: ['L56','L57'], relationshipType: 'supports' }),
  makeLevel(71, "2D Shape Set Identification", ['SK19'], ['SK02'],
    { prerequisites: ['L55','L60'], relationshipType: 'supports' }),
  makeLevel(72, "Calendar Reading", ['SK21'], ['SK10'],
    { prerequisites: ['L44'], relationshipType: 'often_precedes' }),
  makeLevel(73, "Data Handling (Sorting & Tallies)", ['SK23'], ['SK02','SK05'],
    { prerequisites: ['L23'], relationshipType: 'supports' }),
  makeLevel(74, "Number Patterns & Sequences", ['SK18'], ['SK10'],
    { prerequisites: ['L24','L59'], relationshipType: 'often_precedes' }),
  makeLevel(75, "Zero as a Placeholder", ['SK11'], ['SK08'],
    { prerequisites: [
  ...
  makeLevel(76, "Extended Number Line (0-100)", ['SK10'], ['SK09','SK11'],
    { prerequisites: ['L59','L62'], relationshipType: 'often_precedes' }),
  makeLevel(77, "Skip Counting (2s, 5s, 10s)", ['SK18','SK15'], ['SK05'],
    { prerequisites: [
      { levelId: 'L13', relationshipType: 'often_precedes' },
      { levelId: 'L74', relationshipType: 'required_for_procedure',
        rationale: "Skip-counting is a constant-interval number pattern" }] }),

  // ── Class 3 (L78-L91) ───────────────────────────────────────────────────────────────
  makeLevel(78, "3-Digit Place Value & Expanded Form", ['SK11'], ['SK12'],
    { prerequisites: [
      { levelId: 'L63', relationshipType: 'required_for_procedure' },
      { levelId: 'L75', relationshipType: 'required_for_procedure',
        rationale: "Expanded form (e.g. 305 = 300+0+5) requires zero-as-placeholder already in place" }] }),
  makeLevel(79, "Flexible 3-Digit Decomposition", ['SK12'], ['SK11'],
    { prerequisites: [
      { levelId: 'L63', relationshipType: 'required_for_procedure',
        rationale: "3-digit decomposition is the range extension of 2-digit decomposition" },
      { levelId: 'L78', relationshipType: 'required_for_procedure' }] }),
  makeLevel(80, "3-Digit Comparison & Ordering", ['SK09','SK03'], ['SK11'],
    { prerequisites: [
      { levelId: 'L48', relationshipType: 'required_for_procedure',
        rationale: "Class-1 abstract comparison is prerequisite for Class-3 3-digit comparison" },
      { levelId: 'L78', relationshipType: 'often_precedes' }] }),
  makeLevel(81, "Reading & Writing 4-Digit Numbers", ['SK08'], ['SK11'],
    { prerequisites: [
      { levelId: 'L75', relationshipType: 'required_for_procedure',
  ...
  makeLevel(82, "3-Digit Addition & Subtraction Problems", ['SK13','SK14'], ['SK11','SK12'],
    { prerequisites: ['L64','L65','L78'], relationshipType: 'required_for_procedure' }),
  makeLevel(83, "Full Multiplication Tables (2-10)", ['SK15'], ['SK18'],
    { prerequisites: [
      { levelId: 'L68', relationshipType: 'required_for_procedure' },
      { levelId: 'L77', relationshipType: 'required_for_procedure',
        rationale: "Skip-counting is the standard precursor to multiplication-table fluency (same logic as the L58→L61 edge in #278)" }] }),
  makeLevel(84, "Division Facts & Inverse Relation", ['SK16'], ['SK15'],
    { prerequisites: ['L83'], relationshipType: 'required_for_procedure' }),
  makeLevel(85, "Standard Measurement Units", ['SK20'], ['SK24'],
    { prerequisites: ['L70'], relationshipType: 'required_for_procedure' }),
  makeLevel(86, "Relating 2D Faces to 3D Solids", ['SK19'], ['SK04'],
    { prerequisites: [
      { levelId: 'L54', relationshipType: 'required_for_procedure',
        rationale: "Van Hiele L1→L2: composing needs property vocabulary first" },
      { levelId: 'L71', relationshipType: 'supports' }] }),
  makeLevel(87, "Telling Time (Hours & Half-Hours)", ['SK21'], ['SK10'],
    { prerequisites: ['L72'], relationshipType: 'required_for_procedure' }),
  makeLevel(88, "Money Arithmetic", ['SK22'], ['SK13','SK14'],
    { prerequisites: [
      { levelId: 'L45', relationshipType: 'required_for_procedure' },
      { levelId: 'L82', relationshipType: 'often_precedes',
  ...
  makeLevel(89, "Formal Fractions (Half/Quarter)", ['SK17'], ['SK12'],
    { prerequisites: ['L69'], relationshipType: 'required_for_procedure' }),
  makeLevel(90, "Pattern Rules & Generalization", ['SK18'], ['SK24'],
    { prerequisites: ['L74','L83'], relationshipType: 'often_precedes' }),
  makeLevel(91, "Data Handling (Pictographs & Bar Graphs)", ['SK23'], ['SK05','SK09'],
    { prerequisites: ['L73'], relationshipType: 'required_for_procedure' }),

  // ── Class 4 (L92-L109) ───────────────────────────────────────────────────────────────
  makeLevel(92, "4-Digit & 5-Digit Place Value", ['SK11'], ['SK12'],
    { prerequisites: [
      { levelId: 'L78', relationshipType: 'required_for_procedure',
        rationale: "Place-value range extension from 3 digits to 4/5 digits" },
      { levelId: 'L81', relationshipType: 'required_for_procedure' }] }),
  makeLevel(93, "Large Number Operations & Regrouping", ['SK13','SK14','SK11'], ['SK12'],
    { prerequisites: [
      { levelId: 'L82', relationshipType: 'required_for_procedure',
        rationale: "3-digit regrouping is foundational to large-number regrouping" },
      { levelId: 'L92', relationshipType: 'required_for_procedure' }] }),
  makeLevel(94, "Complex Multi-Digit Word Problems", ['SK24','SK13','SK14'], ['SK11','SK12'],
    { prerequisites: ['L93'], relationshipType: 'often_precedes' }),
  makeLevel(95, "Extended Multiplication", ['SK15'], ['SK11','SK12'],
    { prerequisites: [
      { levelId: 'L68', relationshipType: 'required_for_procedure',
        rationale: "Extended multiplication requires basic multiplication-table fluency" },
      { levelId: 'L83', relationshipType: 'required_for_procedure',
  ...
  makeLevel(96, "Formal Long Division", ['SK16'], ['SK15','SK11'],
    { prerequisites: [
      { levelId: 'L84', relationshipType: 'required_for_procedure',
  ...
  makeLevel(97, "Fractional Notation & Equivalence", ['SK17'], ['SK12'],
    { prerequisites: ['L89'], relationshipType: 'required_for_procedure' }),
  makeLevel(98, "Standard Unit Conversion", ['SK20'], ['SK12'],
    { prerequisites: ['L85'], relationshipType: 'required_for_procedure' }),
  makeLevel(99, "Applied Measurement Word Problems", ['SK20','SK24'], ['SK13','SK14'],
    { prerequisites: ['L94','L98'], relationshipType: 'often_precedes' }),
  makeLevel(100, "3D Nets & Spatial Perspective", ['SK19'], ['SK12','SK24'],
    { prerequisites: [
      { levelId: 'L86', relationshipType: 'required_for_procedure',
        rationale: "Van Hiele L2 extended: nets need the same part-whole reasoning" }] }),
  makeLevel(101, "Advanced Time Calculation", ['SK21'], ['SK13','SK14'],
    { prerequisites: ['L87'], relationshipType: 'required_for_procedure' }),
  makeLevel(102, "Complex Money Problems", ['SK22','SK24'], ['SK13','SK14'],
    { prerequisites: ['L88'], relationshipType: 'often_precedes' }),
  makeLevel(103, "Advanced Number Patterns", ['SK18'], ['SK10','SK24'],
    { prerequisites: ['L90'], relationshipType: 'often_precedes' }),
  makeLevel(104, "Bar Graphs & Data Interpretation", ['SK23'], ['SK09','SK24'],
    { prerequisites: ['L91'], relationshipType: 'often_precedes' }),
  makeLevel(105, "Factors & Multiples", ['SK15','SK18'], ['SK16'],
    { prerequisites: [
      { levelId: 'L83', relationshipType: 'required_for_procedure',
        rationale: "Cannot reliably find factors of 24 without knowing what multiplies to 24" },
      { levelId: 'L84', relationshipType: 'often_precedes' }] }),
  makeLevel(106, "Decimals (Tenths & Hundredths)", ['SK08','SK17'], ['SK11','SK12'],
    { prerequisites: [
      { levelId: 'L79', relationshipType: 'supports',
      ...
      { levelId: 'L97', relationshipType: 'required_for_procedure',
        rationale: "Decimals are standardly introduced as an extension of fraction notation" }] }),
  makeLevel(107, "Angles & Turn", ['SK19'], ['SK10'],
    { prerequisites: ['L46'], relationshipType: 'supports' }),
  makeLevel(108, "Symmetry & Reflection", ['SK19'], ['SK18'],
    { prerequisites: ['L90'], relationshipType: 'supports' }),
  makeLevel(109, "Perimeter & Area", ['SK20'], ['SK19','SK13'],
    { prerequisites: [
...
(function sanity(): void {
  ...
  if (LEVEL_SKILL_MAP.length !== 109) {
    throw new Error(`Expected 109 levels, got ${LEVEL_SKILL_MAP.length}`);
  }
  if (Object.keys(SKILL_BY_ID).length !== 24) {
    throw new Error(`Expected 24 core skills, got ${Object.keys(SKILL_BY_ID).length}`);
  }
  if (Object.keys(LEVEL_BY_SCODE).length !== 109) {
    throw new Error(`Expected 109 unique sCode values, got ${Object.keys(LEVEL_BY_SCODE).length} — a level's sCode collided with another's.`);
  }
})();
